...
// Talks to Projects' own API. Every request carries the JWT that was issued
// by FieldClock at login time — Projects' backend verifies that token
// itself (shared JWT_SECRET) rather than looking it up in a shared database.
...
client.interceptors.response.use(
  ...
  async (error) => {
    ...
    try {
      // Refresh happens against FieldClock (the token issuer), not Projects' own API.
